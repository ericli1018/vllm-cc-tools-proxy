...
NODE
test "$(node -p "require('./package.json').version")" = '0.29.4'
test "$(node --input-type=module -e "import('./src/version.js').then((m) => process.stdout.write(m.VERSION))")" = '0.29.4'
...
grep -Fq "visualPromptVersion: 'visual-v14'" src/config.js
grep -Fq "evidenceContractVersion: 'evidence-v10'" src/config.js
...
grep -Fq 'evidence-v10' src/config.js
...
test "$(node -p "require('./package-lock.json').version")" = '0.29.4'
...
# V0.29.4 Generic Zoom Fallback & Vision Contract Repair
 ...
 grep -Fq "visualPromptVersion: 'visual-v14'" src/config.js
 grep -Fq "evidenceContractVersion: 'evidence-v10'" src/config.js
 grep -Fq 'V0.29.4 generic image NEEDS_ZOOM' test/media-adapters.test.js
