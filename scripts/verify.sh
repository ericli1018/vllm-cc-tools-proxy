...
NODE
test "$(node -p "require('./package.json').version")" = '0.30.7'
test "$(node --input-type=module -e "import('./src/version.js').then((m) => process.stdout.write(m.VERSION))")" = '0.30.7'
...
test "$(node -p "require('./package-lock.json').version")" = '0.30.7'
...
# V0.29.16 Diagnostic-First Claude Agent UI Tracing
 ...
 grep -Fq 'claude_agent_request_observed' src/services/proxy-server.js
 grep -Fq 'proxy_progress_first_visible' src/services/proxy-server.js
 grep -Fq 'claude_agent_handoff_observed' src/services/proxy-server.js
 grep -Fq 'describeClaudeAgentRequest' src/proxy/claude-agent-diagnostics.js
 grep -Fq 'describeClaudeAgentHandoff' src/proxy/claude-agent-diagnostics.js
 grep -Fq 'V0.29.16 safely fingerprints Claude Code parent/sub-agent request metadata' test/claude-agent-diagnostics.test.js
 grep -Fq 'V0.29.16 records Agent UI lifecycle diagnostics while preserving V0.29.12 visible progress wire order' test/proxy-server.test.js
 grep -Fq "pipelineVersion: 'media-v8'" src/config.js
 grep -Fq "visualPromptVersion: 'visual-v18'" src/config.js
 grep -Fq "evidenceContractVersion: 'evidence-v14'" src/config.js
...
# V0.29.24 External Context Compact Liveness-Only SSE
 test -f change_log/V0.29.24-更新說明.md
 test -f change_log/V0.29.24-實作與驗證報告.md
 test ! -f CHANGELOG.md
 ! find . -maxdepth 1 -type f \( -name 'V*-更新說明.md' -o -name 'V*-實作與驗證報告.md' -o -name 'V*-診斷說明.md' \) | grep -q .
 grep -Fq 'V0.29.24 Context Compact Liveness-Only SSE' README.md
 grep -Fq 'async function openContextCompactLiveness' src/services/proxy-server.js
 grep -Fq "formatSseEvent('ping', { type: 'ping' })" src/services/proxy-server.js
 grep -Fq 'context_compact_client_stream_open' src/services/proxy-server.js
 grep -Fq 'context_compact_client_stream_stop' src/services/proxy-server.js
 grep -Fq "compactLiveness?.stop('external_compact_complete')" src/services/proxy-server.js
 grep -Fq "compactLiveness?.stop('base_compact_fallback')" src/services/proxy-server.js
 grep -Fq 'V0.29.24 streaming external compact opens ping-only SSE before the delayed summary' test/proxy-server.test.js
 grep -Fq 'V0.29.24 streaming external compact keeps Base compact fallback after ping-only SSE has opened' test/proxy-server.test.js
 grep -Fq 'V0.29.24 adds liveness-only SSE for external Context Compact without changing Main or Sub Agent progress policy' test/deployment.test.js
 grep -Fq "visibleProgressEnabled: claudeAgentRequestContext?.context !== 'subagent'" src/services/proxy-server.js
 grep -Fq "state.sessionId === session && state.agentContext === 'main'" src/proxy/runtime-telemetry.js
 grep -Fq "status_owner: 'main'" src/services/proxy-server.js
 grep -Fq "pipelineVersion: 'media-v8'" src/config.js
...
# V0.29.25 Managed Response Recovery
 test -f change_log/V0.29.25-更新說明.md
 test -f change_log/V0.29.25-實作與驗證報告.md
 test ! -f CHANGELOG.md
 ! find . -maxdepth 1 -type f \( -name 'V*-更新說明.md' -o -name 'V*-實作與驗證報告.md' -o -name 'V*-診斷說明.md' \) | grep -q .
 grep -Fq 'V0.29.25 Managed Response Recovery' README.md
 grep -Fq 'PROXY_MANAGED_RESPONSE_RECOVERY' src/proxy/managed-loop.js
 grep -Fq 'DEFAULT_MAX_STALL_RECOVERY_ROUNDS = 2' src/proxy/managed-loop.js
 grep -Fq 'DEFAULT_TOOL_STALL_TIMEOUT_MS = 300_000' src/proxy/managed-loop.js
 grep -Fq 'managed_model_stall_recovery_started' src/proxy/managed-loop.js
 grep -Fq 'managed_model_stall_recovery_completed' src/proxy/managed-loop.js
 grep -Fq 'onCheckpoint' src/proxy/anthropic-sse-collector.js
 grep -Fq 'completed_blocks' src/proxy/anthropic-sse-collector.js
 grep -Fq "phase: modelRoundProgress.phase || 'waiting'" src/services/proxy-server.js
 grep -Fq 'V0.29.25 collector exposes only completed blocks as a recovery checkpoint while a later tool block is partial' test/anthropic-sse-collector.test.js
 grep -Fq 'V0.29.25 tool phase uses a longer inactivity budget than normal streaming phases' test/managed-loop.test.js
 grep -Fq 'V0.29.25 recovers a stalled tool-phase response from completed blocks and drops duplicate recovered tool calls' test/managed-loop.test.js
 grep -Fq 'V0.29.25 managed proxy recovers a stalled streamed response from completed semantic blocks' test/proxy-server.test.js
 grep -Fq 'V0.29.25 adds phase-aware Managed Response Recovery without new ENV settings' test/deployment.test.js
 ! grep -Eq '^MANAGED_MODEL_TOOL_STALL_TIMEOUT_MS=' .env.example
 ! grep -Eq '^MAX_STALL_RECOVERY_ROUNDS=' .env.example
 grep -Fq "visibleProgressEnabled: claudeAgentRequestContext?.context !== 'subagent'" src/services/proxy-server.js
 grep -Fq 'context_compact_client_stream_open' src/services/proxy-server.js
 grep -Fq "pipelineVersion: 'media-v8'" src/config.js
 grep -Fq "visualPromptVersion: 'visual-v18'" src/config.js
...
# V0.29.27 Server Capability Discovery / ToolSearch Foundation
 test -f change_log/V0.29.27-更新說明.md
 test -f change_log/V0.29.27-實作與驗證報告.md
 grep -Fq 'V0.29.27 Server Capability Discovery / ToolSearch Foundation' README.md
 test -f src/proxy/server-capabilities.js
 grep -Fq 'inspectAnthropicServerCapabilities' src/proxy/server-capabilities.js
 grep -Fq 'inspectAnthropicServerResponse' src/proxy/server-capabilities.js
 grep -Fq 'tool_search_tool_(regex|bm25)' src/proxy/server-capabilities.js
 grep -Fq 'anthropic_server_capability_inventory' src/services/proxy-server.js
 grep -Fq 'tool_search_request_observed' src/services/proxy-server.js
...
# V0.29.30 Native Vision Raw Passthrough
 ...
 test -f change_log/V0.29.30-實作與驗證報告.md
 grep -Fq 'V0.29.30 Native Vision Raw Passthrough' README.md
 grep -Fq 'passthroughPaths' src/proxy/media-preflight.js
 grep -Fq 'native_vision_raw_passthrough_selected' src/proxy/media-adapters.js
 grep -Fq 'passthroughPaths: nativeVisionPassthroughPaths' src/services/proxy-server.js
 grep -Fq 'V0.29.30 keeps Read/direct Native Vision image blocks raw instead of staging and rebuilding them' test/deployment.test.js
 node --test test/native-vision-routing.test.js

# V0.29.29 Native Vision Provenance Routing
 test -f change_log/V0.29.29-更新說明.md
 test -f change_log/V0.29.29-實作與驗證報告.md
 grep -Fq 'V0.29.29 Native Vision Provenance Routing' README.md
 grep -q '^VLLM_BASE_VISION_ENABLED=false$' .env.example
 grep -q '^VISION_NATIVE_PASSTHROUGH=false$' .env.example
 grep -Fq 'VLLM_BASE_VISION_ENABLED: ${VLLM_BASE_VISION_ENABLED:-false}' compose.yaml
 grep -Fq 'VISION_NATIVE_PASSTHROUGH: ${VISION_NATIVE_PASSTHROUGH:-false}' compose.yaml
 grep -Fq "['direct_image', 'read_image'].includes(provenance.sourceKind)" src/proxy/media-adapters.js
 grep -Fq 'native_vision_route_selected' src/services/proxy-server.js
 grep -Fq 'native_vision_base_probe_succeeded' src/services/proxy-server.js
 grep -Fq 'native_vision_fallback_selected' src/services/proxy-server.js
 grep -Fq 'base_image_capability_rejected' src/services/proxy-server.js
 grep -Fq 'V0.29.29 routes ordinary Read/direct images to Base Native Vision while preserving PDF and technical-image Proxy Vision' test/deployment.test.js
 node --test test/native-vision-routing.test.js
 grep -Fq "pipelineVersion: 'media-v8'" src/config.js
 grep -Fq "visualPromptVersion: 'visual-v18'" src/config.js
 grep -Fq "evidenceContractVersion: 'evidence-v14'" src/config.js
...
test -f change_log/V0.29.32-更新說明.md
test -f change_log/V0.29.32-實作與驗證報告.md
...
test -f change_log/V0.29.34-更新說明.md
test -f change_log/V0.29.34-實作與驗證報告.md
grep -Fq 'V0.29.34 PDF Zoom Context Continuity' README.md
test -f src/visual/pdf-zoom-context.js
grep -Fq 'VCC_PDF_ZOOM_CONTEXT' src/visual/pdf-zoom-context.js
...
test -f change_log/V0.29.35-更新說明.md
test -f change_log/V0.29.35-實作與驗證報告.md
grep -Fq 'V0.29.35 Two-Line Streaming Status Preview' README.md
grep -Fq 'value: semanticValue' src/proxy/anthropic-sse-collector.js
grep -Fq 'observePreviewDelta' src/proxy/runtime-telemetry.js
grep -Fq 'PREVIEW_LINE_MAX_CHARS = 2048' src/proxy/runtime-telemetry.js
grep -Fq 'previewPreviousLine' src/services/proxy-server.js
grep -Fq "const cursor = '▌'" scripts/cc-tool-proxy-statusline.js
...
grep -Fq 'V0.29.36 Single-Line Model Progress Timeline' README.md
grep -Fq 'heartbeatRun' src/proxy/progress.js
...
test -f change_log/V0.29.42-更新說明.md
test -f change_log/V0.29.42-實作與驗證報告.md
grep -Fq 'V0.29.42 Canonical Startup Card Ownership' README.md
grep -Fq 'startupBannerCanonicalInteractive' src/services/proxy-server.js
grep -Fq 'plain_request_waiting_for_visible_progress' src/services/proxy-server.js
...
# V0.30.2 Directed Visual Progress Continuation contract
test -f change_log/V0.30.2-更新說明.md
...
grep -Fq 'visual_evidence_state_reused' src/visual/visual-recovery-coordinator.js
grep -Fq 'visual_query_planning_started' src/visual/visual-recovery-coordinator.js
...
node --test --test-name-pattern='V0.30.7 runtime' test/version.test.js

# V0.30.7 source recovery, frontend correlation and bounded evidence contract
test -f src/visual/visual-recovery-store.js
test -f src/visual/visual-recovery-coordinator.js
test -f src/visual/visual-recovery-context.js
test -f change_log/V0.30.7-更新說明.md
grep -Fq 'V0.30.7 Durable Visual Recovery' README.md
node --test test/visual-recovery-store.test.js test/visual-recovery-context.test.js test/visual-recovery-integration.test.js
node --test --test-name-pattern='V0.30.7' test/proxy-server.test.js test/visual-query-planner.test.js
