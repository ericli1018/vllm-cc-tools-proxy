...
export function formatImageEvidence({
  ...
  const output = [
    ...
    ...(originTool ? [field('origin_tool', originTool)] : []),
    ...(sourceKind ? [field('source_kind', sourceKind)] : []),
    ...(readSourceRef ? [field('read_source_ref', readSourceRef)] : []),
    ...(Array.isArray(requestedPages) && requestedPages.length ? [field('requested_pages', requestedPages)] : []),
