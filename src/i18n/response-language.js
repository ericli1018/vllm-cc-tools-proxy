...
const MODEL_TELEMETRY_LABELS = Object.freeze({
  'zh-TW': Object.freeze({ waiting: '模型等待輸出', thinking: '模型思考中', response: '模型回應中', tool: '模型建立工具動作', stalled: '模型資料暫停', noData: '無新資料', total: '總計', startedThinking: '模型開始思考', startedResponse: '模型開始回應', startedTool: '模型建立工具動作' }),
  'zh-CN': Object.freeze({ waiting: '模型等待输出', thinking: '模型思考中', response: '模型响应中', tool: '模型建立工具动作', stalled: '模型数据暂停', noData: '无新数据', total: '总计', startedThinking: '模型开始思考', startedResponse: '模型开始响应', startedTool: '模型建立工具动作' }),
  'en-US': Object.freeze({ waiting: 'Model waiting', thinking: 'Model thinking', response: 'Model responding', tool: 'Model building tool action', stalled: 'Model stalled', noData: 'no upstream data for', total: 'total', startedThinking: 'Model started thinking', startedResponse: 'Model started responding', startedTool: 'Model is building a tool action' }),
  'ja-JP': Object.freeze({ waiting: 'モデル待機中', thinking: 'モデル思考中', response: 'モデル応答中', tool: 'モデルがツール操作を生成中', stalled: 'モデルのデータ受信が停止', noData: '新規データなし', total: '合計', startedThinking: 'モデルが思考を開始', startedResponse: 'モデルが応答を開始', startedTool: 'モデルがツール操作を生成中' }),
  'ko-KP': Object.freeze({ waiting: '모델 출력 대기', thinking: '모델 사고 중', response: '모델 응답 중', tool: '모델 도구 동작 생성 중', stalled: '모델 데이터 정체', noData: '동안 새 데이터 없음', total: '총', startedThinking: '모델 사고 시작', startedResponse: '모델 응답 시작', startedTool: '모델 도구 동작 생성 중' }),
});
...
const PROFILES = Object.freeze({
  'zh-TW': Object.freeze({
    ...
    status: Object.freeze({
      ...
      modelFirstByte: ({ seconds = 0, receivedBytes } = {}) => `模型已開始回傳資料，已執行 ${seconds} 秒${hasReceivedBytes(receivedBytes) ? `（已收到 ${formatReceivedBytes(receivedBytes)}）` : ''}…`,
      ...
      fetchError: ({ host = '網頁' }) => `${host} 讀取失敗；正在交由模型改用其他來源。`,
      queueWait: ({ position = 0, seconds = 0 }) => `正在等待模型執行資源，已排隊 ${seconds} 秒，目前前方有 ${position} 個任務…`,
      queueAdmitted: () => '任務已開始處理…',
      upstreamBusyWait: ({ seconds = 0 } = {}) => seconds > 0 ? `↻ 模型目前忙碌，已等待 ${seconds} 秒；正在等待可用執行資源…` : '↻ 模型目前忙碌，正在等待可用執行資源…',
      upstreamBusyRetry: ({ seconds = 0, attempt = 1 } = {}) => `↻ 模型仍忙碌，已等待 ${seconds} 秒；正在重試第 ${attempt} 次請求…`,
      upstreamBusyAccepted: () => '模型已取得執行資源，開始處理…',
      modelPlanning: () => '正在請模型規劃下一步…',
      modelToolResults: () => '模型正在整理工具結果…',
      modelStallRecovery: ({ attempt = 1, total = 2 } = {}) => `↻ 模型輸出中斷；正在從最近完成狀態恢復（${attempt}/${total}）…`,
      finalChannelRecovery: () => '模型答案通道異常；正在進行一次短格式修正…',
      emptyEndTurnRegeneration: () => '模型本輪未產生有效內容；正在以原始工作上下文重新生成一次…',
      continuationRecovery: ({ candidateChars = 0 } = {}) => candidateChars > 0
        ? `模型尚未形成有效下一步；本輪產生 ${formatCharacterCount(candidateChars)} 字元工作狀態，正在整理並保留續接重點…`
        : '模型尚未形成有效下一步；正在整理並保留本輪工作狀態以進行受控續接…',
      ...
      finalRoundReserved: () => '研究工具預算已停止擴張；保留時間給模型完成下一步…',
      finalLanguageRepair: () => '◇ 模型已完成回答；正在轉換為繁體中文…',
      finalLanguageRepairFallbackBase: () => '◇ 外部語言處理未達要求；正在改由模型完成繁體中文轉換…',
      mediaCacheMiss: () => '正在處理新的文件與圖片內容…',
      mediaReady: () => '文件與圖片內容已就緒；正在交給模型分析…',
      ...
      streamingVisible: () => '模型已開始回傳本輪回答…',
      streamingThinking: () => '模型已開始回傳思考內容…',
      streamingOutput: () => '模型已開始回傳本輪輸出…',
      pdfStart: () => '正在解析 PDF…',
  ...
  'zh-CN': Object.freeze({
    ...
    status: Object.freeze({
      ...
      modelFirstByte: ({ seconds = 0, receivedBytes } = {}) => `模型已开始返回数据，已执行 ${seconds} 秒${hasReceivedBytes(receivedBytes) ? `（已收到 ${formatReceivedBytes(receivedBytes)}）` : ''}…`,
      ...
      fetchError: ({ host = '网页' }) => `${host} 读取失败；正在交由模型改用其他来源。`,
      queueWait: ({ position = 0, seconds = 0 }) => `正在等待模型执行资源，已排队 ${seconds} 秒，目前前方有 ${position} 个任务…`,
      queueAdmitted: () => '任务已开始处理…',
      upstreamBusyWait: ({ seconds = 0 } = {}) => seconds > 0 ? `↻ 模型目前忙碌，已等待 ${seconds} 秒；正在等待可用执行资源…` : '↻ 模型目前忙碌，正在等待可用执行资源…',
      upstreamBusyRetry: ({ seconds = 0, attempt = 1 } = {}) => `↻ 模型仍忙碌，已等待 ${seconds} 秒；正在重试第 ${attempt} 次请求…`,
      upstreamBusyAccepted: () => '模型已取得执行资源，开始处理…',
      modelPlanning: () => '正在请模型规划下一步…',
      modelToolResults: () => '模型正在整理工具结果…',
      modelStallRecovery: ({ attempt = 1, total = 2 } = {}) => `↻ 模型输出中断；正在从最近完成状态恢复（${attempt}/${total}）…`,
      finalChannelRecovery: () => '模型答案通道异常；正在进行一次短格式修正…',
      emptyEndTurnRegeneration: () => '模型本轮未生成有效内容；正在使用原始工作上下文重新生成一次…',
      continuationRecovery: ({ candidateChars = 0 } = {}) => candidateChars > 0
        ? `模型尚未形成有效下一步；本轮生成 ${formatCharacterCount(candidateChars)} 字符工作状态，正在整理并保持续接重点…`
        : '模型尚未形成有效下一步；正在整理并保留本轮工作状态以进行受控续接…',
      continuationStatePreserved: ({ candidateChars = 0, handoffChars = 0, compressed = false } = {}) => compressed
      ...
      finalRoundReserved: () => '研究工具预算已停止扩张；保留时间给模型完成下一步…',
      finalLanguageRepair: () => '◇ 模型已完成回答；正在转换为简体中文…',
      finalLanguageRepairFallbackBase: () => '◇ 外部语言处理未达到要求；正在改由模型完成简体中文转换…',
      mediaCacheMiss: () => '正在处理新的文档与图片内容…',
      mediaReady: () => '文档与图片内容已就绪；正在交给模型分析…',
      ...
      streamingVisible: () => '模型已开始返回本轮回答…',
      streamingThinking: () => '模型已开始返回思考内容…',
      streamingOutput: () => '模型已开始返回本轮输出…',
      pdfStart: () => '正在解析 PDF…',
  ...
  'en-US': Object.freeze({
    ...
    status: Object.freeze({
      ...
      modelFirstByte: ({ seconds = 0, receivedBytes } = {}) => `The model has started returning data. Running for ${seconds}s${hasReceivedBytes(receivedBytes) ? ` (received ${formatReceivedBytes(receivedBytes)})` : ''}…`,
      ...
      fetchError: ({ host = 'web page' }) => `Failed to fetch ${host}; the model will use another source.`,
      queueWait: ({ position = 0, seconds = 0 }) => `Waiting for model capacity; queued for ${seconds}s with ${position} task(s) ahead…`,
      queueAdmitted: () => 'Task processing started…',
      upstreamBusyWait: ({ seconds = 0 } = {}) => seconds > 0 ? `↻ Model is busy; waited ${seconds}s for available execution capacity…` : '↻ Model is busy; waiting for available execution capacity…',
      upstreamBusyRetry: ({ seconds = 0, attempt = 1 } = {}) => `↻ Model is still busy after ${seconds}s; starting attempt ${attempt}…`,
      upstreamBusyAccepted: () => 'Model accepted the request and is starting processing…',
      modelPlanning: () => 'The model is planning the next step…',
      modelToolResults: () => 'The model is processing tool results…',
      modelStallRecovery: ({ attempt = 1, total = 2 } = {}) => `↻ The model stream was interrupted; recovering from the latest completed state (${attempt}/${total})…`,
      finalChannelRecovery: () => 'The model response channel is malformed; applying one short format repair…',
      emptyEndTurnRegeneration: () => 'The model returned no semantic content; regenerating once from the original task context…',
      continuationRecovery: ({ candidateChars = 0 } = {}) => candidateChars > 0
        ? `The model did not form a valid next step; organizing and preserving ${formatCharacterCount(candidateChars)} characters of this round’s model working state…`
        : 'The model did not form a valid next step; preserving this round’s model working state for one controlled continuation…',
      continuationStatePreserved: ({ candidateChars = 0, handoffChars = 0, compressed = false } = {}) => compressed
      ...
      finalRoundReserved: () => 'Research tool expansion stopped; reserving time for the model to finish the next step…',
      finalLanguageRepair: () => '◇ The model finished the answer; converting it to English…',
      finalLanguageRepairFallbackBase: () => '◇ The external language processor did not meet the output contract; switching to the model for English conversion…',
      mediaCacheMiss: () => 'Processing new document and image content…',
      mediaReady: () => 'Document and image content is ready; handing it to the model for analysis…',
      ...
      streamingVisible: () => 'The model started returning this response…',
      streamingThinking: () => 'The model started returning thinking content…',
      streamingOutput: () => 'The model started returning this output…',
  ...
  'ja-JP': Object.freeze({
    ...
    status: Object.freeze({
      ...
      modelFirstByte: ({ seconds = 0, receivedBytes } = {}) => `モデルがデータを返し始めました。実行 ${seconds} 秒${hasReceivedBytes(receivedBytes) ? `（受信 ${formatReceivedBytes(receivedBytes)}）` : ''}…`,
      ...
      fetchError: ({ host = 'Webページ' }) => `${host} の取得に失敗しました。モデルが別の情報源を使用します。`,
      queueWait: ({ position = 0, seconds = 0 }) => `モデルの実行枠を待機中です。${seconds} 秒待機、前に ${position} 件あります…`,
      queueAdmitted: () => 'タスクの処理を開始しました…',
      upstreamBusyWait: ({ seconds = 0 } = {}) => seconds > 0 ? `↻ モデルが混雑中です。${seconds} 秒待機し、実行枠を待っています…` : '↻ モデルが混雑中です。利用可能な実行枠を待っています…',
      upstreamBusyRetry: ({ seconds = 0, attempt = 1 } = {}) => `↻ モデルはまだ混雑中です。${seconds} 秒待機後、第 ${attempt} 回を再試行します…`,
      upstreamBusyAccepted: () => 'モデルが要求を受理し、処理を開始しました…',
      modelPlanning: () => 'モデルが次の手順を計画しています…',
      modelToolResults: () => 'モデルがツールの結果を処理しています…',
      modelStallRecovery: ({ attempt = 1, total = 2 } = {}) => `↻ モデルの出力が中断しました。直近の完了状態から復旧しています（${attempt}/${total}）…`,
      finalChannelRecovery: () => 'モデルの応答チャネルに異常があります。短い形式修正を1回実行します…',
      emptyEndTurnRegeneration: () => 'モデルが有効な内容を返さなかったため、元の作業コンテキストから1回だけ再生成します…',
      continuationRecovery: ({ candidateChars = 0 } = {}) => candidateChars > 0
        ? `モデルが有効な次の手順を形成できませんでした。このラウンドの作業状態 ${formatCharacterCount(candidateChars)} 文字を整理・保持しています…`
        : 'モデルが有効な次の手順を形成できませんでした。このラウンドの作業状態を保持して継続処理を準備しています…',
      continuationStatePreserved: ({ candidateChars = 0, handoffChars = 0, compressed = false } = {}) => compressed
      ...
      finalRoundReserved: () => '調査ツールの拡張を停止し、モデルが次の手順を完了する時間を確保します…',
      finalLanguageRepair: () => '◇ モデルの回答が完了しました。日本語に変換しています…',
      finalLanguageRepairFallbackBase: () => '◇ 外部言語処理が要件を満たさなかったため、モデルで日本語変換を続行しています…',
      mediaCacheMiss: () => '新しい文書と画像の内容を処理しています…',
      mediaReady: () => '文書と画像の内容を準備しました。モデルに渡して分析します…',
      ...
      streamingVisible: () => 'モデルが応答の返却を開始しました…',
      streamingThinking: () => 'モデルが思考内容の返却を開始しました…',
      streamingOutput: () => 'モデルが出力の返却を開始しました…',
  ...
  'ko-KP': Object.freeze({
    ...
    status: Object.freeze({
      ...
      modelFirstByte: ({ seconds = 0, receivedBytes } = {}) => `모델이 데이터를 반환하기 시작했습니다. ${seconds}초 실행${hasReceivedBytes(receivedBytes) ? ` (수신 ${formatReceivedBytes(receivedBytes)})` : ''}…`,
      ...
      fetchError: ({ host = '웹 페이지' }) => `${host} 가져오기에 실패했습니다. 모델이 다른 자료원을 사용합니다.`,
      queueWait: ({ position = 0, seconds = 0 }) => `모델 실행 자원을 기다리고 있습니다. ${seconds}초 대기, 앞에 ${position}개 작업이 있습니다…`,
      queueAdmitted: () => '작업 처리를 시작했습니다…',
      upstreamBusyWait: ({ seconds = 0 } = {}) => seconds > 0 ? `↻ 모델이 혼잡합니다. ${seconds}초 기다렸으며 실행 자원을 기다리는 중입니다…` : '↻ 모델이 혼잡합니다. 사용 가능한 실행 자원을 기다리는 중입니다…',
      upstreamBusyRetry: ({ seconds = 0, attempt = 1 } = {}) => `↻ 모델이 여전히 혼잡합니다. ${seconds}초 대기 후 ${attempt}번째 시도를 진행합니다…`,
      upstreamBusyAccepted: () => '모델이 요청을 수락하여 처리를 시작합니다…',
      modelPlanning: () => '모델이 다음 단계를 계획하고 있습니다…',
      modelToolResults: () => '모델이 도구 실행 결과를 처리하고 있습니다…',
      modelStallRecovery: ({ attempt = 1, total = 2 } = {}) => `↻ 모델 출력이 중단되었습니다. 최근 완료 상태에서 복구 중입니다 (${attempt}/${total})…`,
      finalChannelRecovery: () => '모델 응답 채널에 이상이 있습니다. 짧은 형식 수정을 한 번 수행합니다…',
      emptyEndTurnRegeneration: () => '모델이 유효한 내용을 생성하지 않아 원래 작업 컨텍스트에서 한 번 다시 생성합니다…',
      continuationRecovery: ({ candidateChars = 0 } = {}) => candidateChars > 0
        ? `모델이 유효한 다음 단계를 만들지 못했습니다. 이번 라운드의 모델 작업 상태 ${formatCharacterCount(candidateChars)}자를 정리하고 보존하고 있습니다…`
        : '모델이 유효한 다음 단계를 만들지 못했습니다. 이번 라운드의 모델 작업 상태를 보존해 제한된 이어쓰기를 준비합니다…',
      continuationStatePreserved: ({ candidateChars = 0, handoffChars = 0, compressed = false } = {}) => compressed
      ...
      finalRoundReserved: () => '조사 도구 확장을 중단하고 모델이 다음 단계를 완료할 시간을 확보합니다…',
      finalLanguageRepair: () => '◇ 모델 답변이 완료되었습니다. 한국어로 변환하고 있습니다…',
      finalLanguageRepairFallbackBase: () => '◇ 외부 언어 처리 결과가 요구 사항을 충족하지 않아 모델로 한국어 변환을 계속합니다…',
      mediaCacheMiss: () => '새 문서와 그림 내용을 처리하고 있습니다…',
      mediaReady: () => '문서와 그림 내용이 준비되었습니다. 모델에 넘겨 분석합니다…',
      ...
      streamingVisible: () => '모델이 응답 반환을 시작했습니다…',
      streamingThinking: () => '모델이 사고 내용 반환을 시작했습니다…',
      streamingOutput: () => '모델이 출력 반환을 시작했습니다…',
...
export function formatProgressClock(timestampMs = Date.now()) {
  const date = new Date(Number.isFinite(Number(timestampMs)) ? Number(timestampMs) : Date.now());
...
export function progressBlockHeader(locale, { timeText = '', timestampMs } = {}) {
  const label = languageProfile(resolveResponseLanguage(locale)).progressHeader;
