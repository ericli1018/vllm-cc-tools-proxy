...
test('loadConfig exposes the five vLLM settings and one proxy mode', () => {
  ...
  assert.equal(config.cache.visualPromptVersion, 'visual-v17');
  assert.equal(config.cache.evidenceContractVersion, 'evidence-v13');
...
test('Base and managed requests have no Proxy concurrency limit; only Vision keeps an auxiliary limit', () => {
  const config = loadConfig({ VLLM_BASE_URL: 'http://vllm:8000' });
  assert.deepEqual(config.concurrency, { visionLimit: 1 });
  assert.equal(config.vllmBusyRetryIntervalMs, 15000);
});

test('large resource profile raises only the default Vision auxiliary concurrency', () => {
  ...
  assert.deepEqual(small.concurrency, { visionLimit: 1 });
  assert.deepEqual(large.concurrency, { visionLimit: 2 });
});

test('VISION_MAX_CONCURRENCY remains configurable while legacy managed queue ENV no longer affects routing', () => {
  const config = loadConfig({
    VLLM_BASE_URL: 'http://vllm:8000',
    CONCURRENCY_PROFILE: 'small',
    MANAGED_MAX_CONCURRENCY: '1',
    MANAGED_MAX_QUEUE: '0',
    MANAGED_QUEUE_TIMEOUT_MS: '1000',
    VISION_MAX_CONCURRENCY: '2',
  });
  assert.deepEqual(config.concurrency, { visionLimit: 2 });
});

test('invalid Vision concurrency is rejected', () => {
...
test('visual provider and thinking mode are explicit and strictly validated', () => {
  ...
  assert.equal(ollama.vllmVisionTimeoutMs, 120000);

  const customTimeout = loadConfig({
    VLLM_BASE_URL: 'http://vllm:8000',
    VLLM_VISION_TIMEOUT_MS: '45000',
  });
  assert.equal(customTimeout.vllmVisionTimeoutMs, 45000);

  assert.throws(() => loadConfig({ VLLM_BASE_URL: 'http://vllm:8000', VLLM_VISION_PROVIDER: 'unknown' }), /VLLM_VISION_PROVIDER/);
  assert.throws(() => loadConfig({ VLLM_BASE_URL: 'http://vllm:8000', VLLM_VISION_TIMEOUT_MS: '999' }), /VLLM_VISION_TIMEOUT_MS/);
...
test('V0.29.7 bumps visual and evidence cache generations for failure-aware recovery contract', () => {
  const config = loadConfig({ VLLM_BASE_URL: 'http://vllm:8000' });
  assert.equal(config.cache.pipelineVersion, 'media-v8');
  assert.equal(config.cache.visualPromptVersion, 'visual-v17');
  assert.equal(config.cache.evidenceContractVersion, 'evidence-v13');
