...
test('V0.29.44 directed mode does not override Native Vision raw passthrough priority', async () => {
  const png = await fs.readFile(new URL('./fixtures/text-image.png', import.meta.url));
  let visionCalls = 0;
  const adapters = createMediaAdapters(baseAdapterConfig({
    vllmBaseVisionEnabled: true,
    visionNativePassthrough: true,
    visionOrchestrationMode: 'directed',
  }), undefined, undefined, {
    mediaProgress: fakeProgress('direct_image'),
    normalizeImage: async (buffer) => ({ buffer, mediaType: 'image/png', width: 600, height: 180 }),
    analyzeVisualAssets: async () => { visionCalls += 1; return { markdown: 'SHOULD NOT RUN' }; },
  });
  const output = await adapters.adaptImage({
    type: 'image', source: { type: 'base64', media_type: 'image/png', data: png.toString('base64') },
  }, { path: ['messages', 0, 'content', 0] });
  assert.equal(output.type, 'image');
  assert.equal(output.source.data, png.toString('base64'));
  assert.equal(visionCalls, 0);
});
