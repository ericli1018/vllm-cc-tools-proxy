...
test('proxy health endpoint reports diagnostic release, admission and cache state', async (t) => {
  ...
  assert.deepEqual(await response.json(), {
    status: 'ok', service: 'proxy', version: '0.29.2', revision: 'test',
...
test('V0.2.28.12 shows one runtime startup banner per Claude Code session without sending it upstream', async (t) => {
  ...
  assert.match(first, /VERSION\s+0\.29\.2/);
...
test('V0.2.28.17 read-only session status endpoint returns semantic telemetry without contacting Base vLLM', async (t) => {
  ...
  assert.equal(payload.version, '0.29.2');
  assert.equal(payload.session_id, 'status-s1');
  assert.equal(payload.phase, 'thinking');
  assert.match(payload.display, /CC TOOL PROXY 0\.29\.2/);
