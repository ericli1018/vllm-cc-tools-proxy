...
test('proxy health endpoint reports diagnostic release, admission and cache state', async (t) => {
  ...
  assert.deepEqual(await response.json(), {
    status: 'ok', service: 'proxy', version: '0.29.31', revision: 'test',
...
test('streamed media progress shows filename and semantic heartbeats across delayed Base vLLM TTFT', async (t) => {
  ...
  assert.match(stream, /(?:◌ 主模型等待輸出|[◐◓◑◒] 主模型思考中|◆ 主模型回應中|⚠ 主模型資料暫停)/);
...
test('V0.2.27.3 continuation visible progress resets received bytes for the new model round', async (t) => {
  ...
  assert.match(continuationStream, /◌ 主模型等待輸出 · \d+s · 0 B/);
  assert.doesNotMatch(continuationStream, /◌ 主模型等待輸出 · \d+s · [0-9.]+ KB/);
...
test('V0.2.28.14 reports a post-first-byte upstream stall without retrying the accepted request', async (t) => {
  ...
  const stream = await response.text();
  assert.equal(response.status, 200);
  assert.match(stream, /⚠ 主模型資料暫停 · \d+s 無新資料 · 總計 [^\\n]+/);
...
test('V0.2.28.7 managed model progress exposes compact thinking and response phases', async (t) => {
  ...
  assert.match(stream, /(?:[◐◓◑◒] 主模型思考中|◆ 主模型回應中|⚠ 主模型資料暫停) · [^\n"]+/);
  assert.match(stream, /◆ 主模型回應中 · \d+s · [^\n"]+/);
...
test('V0.2.28.12 shows one runtime startup banner per Claude Code session without sending it upstream', async (t) => {
  ...
  assert.match(first, /VERSION\s+0\.29\.31/);
...
test('V0.2.28.17 read-only session status endpoint returns semantic telemetry without contacting Base vLLM', async (t) => {
  ...
  assert.equal(payload.version, '0.29.31');
  assert.equal(payload.session_id, 'status-s1');
  assert.equal(payload.phase, 'thinking');
  assert.match(payload.display, /CC TOOL PROXY 0\.29\.31/);
