...
## V0.29.15 Always-on Transport Liveness and Deferred Parent Progress

V0.29.15 makes the connection-liveness requirement independent from assistant semantic content. Every streaming Claude Code request still opens an Anthropic SSE message immediately, emits an immediate `event: ping`, and keeps emitting `event: ping` according to `PROGRESS_PING_INTERVAL_MS` even while Parent semantic progress is deferred. This transport heartbeat is never disabled merely because the request declares `Agent` or legacy `Task`. The purpose is to guarantee outbound activity so a slow local backend does not look like a dead or disconnected request.

For a top-level Parent/Main request that can dispatch `Agent`/`Task`, Proxy semantic progress now starts in **deferred semantic progress** mode rather than V0.29.14's blanket suppression. Progress state, runtime telemetry, native `statusLine`, usage tracking, and transport pings continue immediately. If the first decisive upstream block is ordinary text or a non-Agent tool, Proxy releases the latest pending `目前處理進度：…` state and then continues normal visible progress. If the decisive block is `Agent` or `Task`, the pending synthetic text is discarded while transport pings continue, so the native Agent/Task `tool_use` remains content **index 0** and Claude Code keeps the real sub-agent working name. Thinking blocks do not resolve the gate because an Agent handoff may still follow them.

Managed internal loops are more conservative: a Proxy-managed WebSearch/WebFetch round does not release Parent semantic progress because a later model round may still end in `Agent`/`Task`. The gate is resolved from the terminal response. This preserves title correctness without creating a silent HTTP/SSE connection.

Sub Agent execution requests identified by `x-claude-code-agent-id` / `x-claude-code-parent-agent-id` keep ordinary visible semantic progress, including `目前處理進度：…`, while still using the same transport heartbeat. Sub Agent requests do not claim the one-time Parent startup banner. No new ENV is added; the existing `PROGRESS_PING_INTERVAL_MS` controls the recurring transport heartbeat. Persistent cache generations remain `media-v8`, `visual-v18`, and `evidence-v14`.
