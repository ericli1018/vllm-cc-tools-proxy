...
V0.29.30 tightens the V0.29.29 Native Vision contract. With both settings enabled:
...
Claude Code `Read(image)` (`read_image`) and ordinary `direct_image` blocks are selected by provenance **before** media staging. Their original `tool_use -> tool_result -> image` conversation structure and original image content block are forwarded to Base vLLM unchanged. The Proxy does not decode the selected image into a temporary `proxy_file`, does not rehydrate/re-encode it, and does not replace it with visual evidence. In other words, there is no `proxy_file` rehydration on the normal Native Vision path.

The Proxy still performs its normal request routing, safety boundaries, model selection, progress/liveness, and managed-tool handling around the request. "Raw passthrough" specifically means that eligible image content and its Claude Code conversation context are not rewritten by the media/Vision pipeline before Base vLLM sees them.

`read_pdf_image`, PDF documents, and generic `tool_result_image` remain on the existing Proxy Vision/evidence path. Mixed requests preserve eligible raw Native Vision images while independently transforming only the PDF/technical media that require Proxy processing. If Base explicitly rejects image/vision input during the existing capability preflight, the request may still fall back to the configured `VLLM_VISION_*` pipeline; ordinary Base 5xx/network failures are not treated as capability rejection.

No new ENV variables are added. `media-v8`, `visual-v18`, and `evidence-v14` remain unchanged. Local ToolSearch, WebSearch/WebFetch, Main Agent visible progress, Sub Agent liveness-only behavior, Context Compact ping-only liveness, and Managed Response Recovery are unchanged.
...
Sub Agent request
└─ Proxy progress → thinking block / thinking_delta

Claude Code native Sub Agent task row
└─ no Proxy row override, no title prefix, no subagentStatusLine
```

The Sub Agent carrier is selected only after Claude Code request metadata identifies `agent_context=subagent`. This prevents Proxy-generated `目前處理進度：...` text from becoming ordinary assistant text that Claude Code can reuse as the latest Sub Agent activity/title summary. The model/tool lifecycle itself is not suppressed or deferred. WebSearch/tool-result continuation requests reopen the same Sub Agent thinking-progress carrier on every turn.

`stripProgressHistory()` recognizes both historical text-based Proxy progress and the V0.29.21 synthetic thinking-progress block. The synthetic progress block is removed before model reuse, so an unsigned Proxy-owned thinking block cannot enter the next Base-model prompt. Real model thinking blocks and signatures are preserved unchanged.

The global Claude Code `statusLine` remains **Main-owned** from V0.29.20: Proxy-wide counters include Main and Sub Agent work, but the phase/elapsed/bytes portion shown by `◆ CC TOOL PROXY ...` is selected only from Main request telemetry. V0.29.21 does not restore the retired V0.29.19 `subagentStatusLine` hook and does not ship any Sub Agent row renderer.
...
V0.29.18 keeps the V0.29.17 `SubagentDisplayRegistry` and changes only title-bearing progress rendering. When `progressTitle` is bound for a Sub Agent, every visible progress delta repeats the stable Agent description, the localized progress header, and the newest progress message. This targets Claude Code TUI behavior observed in real logs where a Sub Agent row starts correctly but later changes to `目前處理進度：…` after a 30-second semantic heartbeat or later phase delta.

Main Agent progress is intentionally unchanged: without `progressTitle`, only the first visible delta contains `目前處理進度：`, and subsequent deltas remain append-only status lines. Sub Agent progress remains visible and transport heartbeat behavior is unchanged. Because Anthropic `text_delta` is append-only, the Sub Agent transcript will contain repeated title/header groups; this is an intentional compatibility trade-off for this release.
...
V0.29.16 is rebuilt **directly from the V0.29.12 release baseline**. V0.29.13, V0.29.14, and V0.29.15 introduced experimental Agent/sub-agent progress suppression and deferral policies; none of those policies are carried into this release. **V0.29.12 visible progress semantics are preserved** for both the main Claude Code view and sub-agent execution views.

This release does not attempt another speculative UI fix. Instead, it adds safe correlation diagnostics around the exact lifecycle that produced the original symptom where Claude Code displayed `目前處理進度：…` in place of a sub-agent work name.

The new events are:

- `claude_agent_request_observed`: records whether the request has Claude Code agent/parent-agent headers, short SHA-256 fingerprints for correlation, declared `Agent`/`Task` tool names, stream mode, and message count.
- `proxy_progress_first_visible`: records when the Proxy first creates its visible progress content block, including phase, sequence, elapsed time, agent context, and the content index used by the unchanged V0.29.12 progress stream.
- `claude_agent_handoff_observed`: records when a final Base response contains `Agent` or legacy `Task` tool-use blocks, their response block indexes, short fingerprints for tool-use ids and descriptions, description length, stop reason, and whether visible Proxy progress had already appeared.

These events intentionally do **not** log API keys, authorization headers, prompts, message contents, Agent prompt fields, literal Agent descriptions, literal agent ids, or literal tool-use ids. Correlation-sensitive values are represented only by 12-hex SHA-256 prefixes plus lengths where useful.

The Agent UI diagnostic layer is observational only. It does not alter `ProgressStream.update()`, content-block numbering, `Agent.input`, `Task.input`, main/sub-agent request routing, or the V0.29.12 memory lifecycle fixes. Persistent cache generations remain `media-v8`, `visual-v18`, and `evidence-v14`, and no new ENV is added.
